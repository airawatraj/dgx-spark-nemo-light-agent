...
# ///
"""
DGX Spark / Nemotron-3.5-Lightning-30B Benchmark
Tests TPS, TTFT, concurrency, and context window scaling up to 1M tokens.
...
def test_concurrent(host, port, model, max_concurrent=4):
    ...
    for n in range(1, max_concurrent + 1):
        ...
        if valid:
            ...
            color = "green" if per_session >= 15 else "yellow" if per_session >= 8 else "red"
...
def test_context_window(host, port, model):
    ...
    for size in sizes:
        ...
        # Allow extra timeout for very large context windows (1M context might take longer to process prefill)
        timeout_sec = 240 if size >= 262144 else 120
        ...
        if err:
            ...
            tps_color = "green" if tps >= 15 else "yellow" if tps >= 8 else "red"
...
def print_summary(avg_tps, peak_tps, max_context, host, port, model):
    ...
    result_line("Peak TPS (single session)", f"{peak_tps}", "tok/s",
                "green" if peak_tps >= 22 else "yellow" if peak_tps >= 15 else "red")
    result_line("Max usable context", f"~{max_context:,}" if max_context else "not tested", "tokens")
    print()
    if avg_tps >= 20:
        print(f"  {c('★ Excellent — high throughput served successfully.', 'green')}")
    elif avg_tps >= 12:
        print(f"  {c('✓ Good — solid baseline performance.', 'yellow')}")
    else:
        print(f"  {c('✗ Performance below expectations — check logs/swap', 'red')}")
    print()
...
def main():
    ...
    print(f"\n{c('DGX Spark Nemotron-3.5-Lightning-30B Benchmark', 'bold')}")
